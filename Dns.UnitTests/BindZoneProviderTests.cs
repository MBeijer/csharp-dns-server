...
public class BindZoneProviderTests
{
	...
	public void GenerateZone_ReturnsZoneRecordsFromBindFile()
	{
		...
		var filteredRecords =
			zone.Records.Where(record => record.Host == "www.example.com" && record.Type == ResourceType.A);

		var wwwA = Assert.Single(filteredRecords);
		Assert.Equal("192.0.2.10", Assert.Single(wwwA.Addresses));

		filteredRecords =
			zone.Records.Where(record => record.Host == "www.example.com" && record.Type == ResourceType.AAAA);
