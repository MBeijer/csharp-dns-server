...
public sealed class DnsCliHostFixture : IAsyncLifetime, IDisposable
{
	...
	public string BuildHostName(string hostPrefix)
	{
		...
		if (hostPrefix.EndsWith(ZoneSuffix, StringComparison.OrdinalIgnoreCase)) return hostPrefix;
	...
	private void ValidateArtifacts()
	{
		if (!File.Exists(TestProjectPaths.DnsCliDllPath))
			throw new FileNotFoundException(
				"dns-cli binary not found. Run dotnet build before executing the integration tests.",
				TestProjectPaths.DnsCliDllPath
			);
	...
	private void CopyZoneFile()
	{
		...
		File.Copy(GetZoneSourcePath(), destination, true);
	}
	...
	private void WriteConfigFile(int dnsPort, int httpPort)
	{
		...
		template = template.Replace(
			"{{DNS_PORT}}",
			dnsPort.ToString(CultureInfo.InvariantCulture),
		...
		template = template.Replace(
			"{{ZONE_FILE}}",
			JsonEncodedText.Encode(zoneFilePath).ToString(),
			StringComparison.Ordinal
		);
	...
	private void StartProcess()
	{
		...
		if (_process.HasExited) throw new InvalidOperationException("dns-cli exited immediately after start.");
		...
		_process.Exited += (_, __) =>
		{
			if (!_readyTcs.Task.IsCompleted)
				_readyTcs.TrySetException(
					new InvalidOperationException("dns-cli exited before it signaled readiness.")
				);
		};
	...
	private async Task PumpStreamAsync(StreamReader reader, string prefix)
	{
		while (await reader.ReadLineAsync().ConfigureAwait(false) is { } line)
		{
			...
			if (line.IndexOf("Zone reloaded", StringComparison.OrdinalIgnoreCase) >= 0) _readyTcs.TrySetResult(true);
	...
	private void CleanupWorkingDirectory()
	{
		try
		{
			if (_workingDirectory != null && _workingDirectory.Exists) _workingDirectory.Delete(true);
		}
		catch
		{
			// ignored
