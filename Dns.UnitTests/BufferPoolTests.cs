...
public class BufferPoolTests
{
	...
	public void RentBuffer_DisposedBuffer_CanBeRentedAgain()
	{
		...
		for (var i = 0; i < 10; i++) buffers.Add(BufferPool.RentBuffer());
	...
	public void RentBuffer_BufferContentsAreWritable()
	{
		...
		// Write test pattern
		for (var i = 0; i < 512; i++) span[i] = (byte)(i % 256);

		// Verify contents
		for (var i = 0; i < 512; i++) Assert.Equal((byte)(i % 256), span[i]);
	}
	...
	public async Task RentBuffer_HighThroughput_NoMemoryLeak()
	{
		...
		Assert.True(
			memoryGrowth < 10 * 1024 * 1024, // Less than 10MB growth
	...
	public async Task PooledMemoryStream_HighThroughput_PoolReuse()
	{
		...
		Assert.True(
			seenInstances.Count < iterations,
