...
public class EndianTests
{
	...
	public void DnsMessage_Parse_HandlesNetworkByteOrder()
	{
		...
		{
			0x12,
			0x34, // Query ID: 0x1234 (big-endian)
			0x01,
			0x00, // Flags: standard query, recursion desired
			0x00,
			0x01, // Question count: 1 (big-endian)
			0x00,
			0x00, // Answer count: 0
			0x00,
			0x00, // Authority count: 0
			0x00,
			...
			0x00,
			0x01, // Type A (big-endian)
			0x00,
			0x01, // Class IN (big-endian)
		};
	...
	public void BinaryPrimitives_ReadBigEndian_MatchesDnsProtocol()
	{
		...
		offset = 0;
		var value32 = DnsProtocol.ReadUint(networkData, ref offset).SwapEndian();
