...
describe("DnsApiClient", () => {
    ...
    it("surfaces MVC validation messages when a zone save is rejected", async () => {
        const fetchMock = vi.fn().mockResolvedValue({
            ok: false,
            status: 400,
            json: async () => ({
                ...
                errors: {
                    "Records[0].Data": ["A record 'www' must contain a valid IPv4 address. Use an AAAA record for IPv6 addresses."]
                }
            })
        });
        vi.stubGlobal("fetch", fetchMock);
        ...
        const client = new DnsApiClient("http://api.local");
        await expect(client.createZone({
            suffix: "example.com",
            records: [{host: "www", type: "A", data: "2001:db8::10"}]
        })).rejects.toThrow("Use an AAAA record");
    });
