...
public class DnsProtocol
{
	...
	public static bool TryParse(byte[] bytes, out DnsMessage dnsMessage) =>
		TryParse(bytes, bytes.Length, out dnsMessage);
	...
	{
		if (!DnsMessage.TryParse(bytes, length, out dnsMessage)) return false;
	...
	{
		var ret = BinaryPrimitives.ReadUInt16BigEndian(bytes[offset..]);
		offset += sizeof(ushort);
		return ret;
	}
	...
	{
		var ret = BinaryPrimitives.ReadUInt32BigEndian(bytes[offset..]);
		offset += sizeof(uint);
		return ret;
	}
	...
	public static string ReadStringOptimized(ReadOnlySpan<byte> bytes, ref int currentOffset)
	{
		...
		while (true)
		{
			...
			if (segmentLength > 63) throw new InvalidDataException("DNS label length exceeds maximum of 63 bytes.");
			...
			var requiredLength = nameLength + segmentLength + (nameLength > 0 ? 1 : 0);
			...
			if (nameLength > 0) nameBuffer[nameLength++] = '.';

			// Copy label bytes directly to char buffer (ASCII only)
			var labelBytes = bytes.Slice(readOffset, segmentLength);
			for (var i = 0; i < segmentLength; i++)
			{
				var b = labelBytes[i];
				if (b > 0x7F)
					throw new InvalidDataException(
						"DNS label contains non-ASCII characters, which are not allowed per RFC 1035."
					);
    ...
    public static string ReadStringLegacy(byte[] bytes, ref int currentOffset)
	{
		...
		while (true)
		{
			...
			var label = Encoding.ASCII.GetString(bytes, readOffset, segmentLength);
