...
/// </summary>
public class IPProbeZoneProvider(ILogger<IPProbeZoneProvider> logger, IDnsResolver resolver)
	: BaseZoneProvider(resolver)
{
	...
	private IEnumerable<ZoneRecord> GetZoneRecords(State s)
	{
		...
		{
			var availableAddresses = host.AddressProbes.Where(addr => addr.IsAvailable).Select(addr => addr.Address);
